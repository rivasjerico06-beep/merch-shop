...
export default function AgentLeadsPage() {
  ...
  const createOwnLead = async (event: React.FormEvent) => {
    ...
    const response = await fetch("/api/agent/leads", {
      ...
      body: JSON.stringify({
        ...
        phone,
        email: email || null,
        product_interest: newLeadForm.product_interest.trim() || null,
        contact_basis: newLeadForm.contact_basis,
        contact_basis_details: details,
        consent_reference:
          newLeadForm.contact_basis === "documented_consent"
    ...
    if (!response.ok) {
      addToast(
        response.status === 429 || payload.error === "RATE_LIMITED"
          ? "Too many lead actions. Please try again shortly."
          : payload.message || "Unable to create lead.",
        "error"
      );
      setCreatingLead(false);
      return;
    ...
    addToast(payload.message || "Lead created and ready for your assisted workflow.", "success");
  ...
  const submitReviewRequest = async (event: React.FormEvent) => {
    event.preventDefault();

    const reason = reviewRequestForm.reason.trim();
    const phone = reviewRequestForm.phone.trim();

    if (reason.length < 10 || reason.length > 1000) {
      addToast("Explain the issue using 10 to 1,000 characters.", "error");
      return;
    }

    if (phone && (phone.length > 30 || !/^[+0-9() -]+$/.test(phone))) {
      addToast("Enter a valid phone number or leave it blank.", "error");
      return;
    }
    ...
    const response = await fetch("/api/agent/leads", {
      ...
      body: JSON.stringify({
        ...
        request_type: reviewRequestForm.request_type,
        notes: reason,
        phone: phone || null,
        related_lead_id: reviewRequestForm.related_lead_id || null,
      }),
    });
    ...
    if (!response.ok) {
      addToast(
        response.status === 429 || payload.error === "RATE_LIMITED"
          ? "Too many lead actions. Please try again shortly."
          : payload.message || "Unable to send admin review request.",
        "error"
      );
      setSubmittingReview(false);
      return;
    ...
    addToast(payload.message || "Admin review request submitted.", "success");
  ...
  const markDoNotContact = async (lead: Lead) => {
    ...
    if (!response.ok) {
      addToast(
        response.status === 429 || payload.error === "RATE_LIMITED"
          ? "Too many lead actions. Please try again shortly."
          : payload.message || "Unable to mark Do Not Contact.",
        "error"
      );
      return;
    }
